...
def read_metadata(path: str) -> Dict[str, str]:
    ...
    """
    cmd = [_exiftool_cmd(), "-n", "-s", "-s", "-s", "-json"] + [f"-{t}" for t in sorted({t for lst in TAG_MAP.values() for t in lst})] + [path]
    rc, out, err = _run(cmd)
    
    if rc != 0:
        if not ensure_exiftool_available():
            logger.error("ExifTool is not installed or runnable.")
            raise RuntimeError(f"exiftool is not installed or runnable. Path resolved to: '{_exiftool_cmd()}'. Error: {err.strip()}")
...
def parse_decimal_or_dms(val: str) -> float:
    try:
        ...
        pass
    match = re.search(r'''(?:(?P<dir_front>[NSEW])\s*)?(?P<deg>-?\d+(?:\.\d+)?)\s*[°d]?\s*(?:(?P<min>\d+(?:\.\d+)?)\s*['m]?\s*)?(?:(?P<sec>\d+(?:\.\d+)?)\s*["s]?\s*)?(?P<dir_back>[NSEW])?''', val, re.IGNORECASE)
    if not match:
...
def _tag_assignments(payload: Dict[str, str], skip_keywords: bool = False) -> List[str]:
    ...
    # Process GPS specially first to avoid numeric '-n' string issues
    ...
    for field, value in payload.items():
        ...
        # Always write to all mapped tags to prevent Windows from seeing older leftover
        # XMP or IPTC properties if we only updated the EXIF tags, achieving maximum sync.
        target_tags = tags
...
def _create_backups(paths: str | List[str]) -> None:
    ...
    for p in path_list:
        ...
        # Don't overwrite existing backups to preserve the true original
...
def write_metadata(paths: str | List[str], payload: Dict[str, str], skip_keywords: bool = False, inplace: bool = False) -> None:
    if not ensure_exiftool_available():
    ...
    # Always overwrite original to avoid exiftool creating _original files locally (handled by our backup dir now)
...
def clear_metadata(paths: str | List[str], fields: List[str], inplace: bool = False) -> None:
    if not ensure_exiftool_available():
        raise RuntimeError("exiftool is not installed.")
    ...
    for field in fields:
        ...
        # Special case: Clearing GPS coordinates also requires clearing their Ref counterparts
